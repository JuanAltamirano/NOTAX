...
function Home() {
  ...
  const calcular = () => {
    ...
    const acumulado = (n1 * p1) / 100 + (n2 * p2) / 100 + (n3 * p3) / 100;
    const restante = 100 - porcentajeUsado;
    const necesaria = (3.0 - acumulado) / (restante / 100);

    setResultado(`Necesitas sacar ${necesaria.toFixed(2)} en el ${restante}% restante para aprobar.`);
  };
  ...
  return (
    <div>
      ...
      <div className="hero">
        ...
        <p>Con NOTAX puedes registrar tus materias, calcular tu promedio y saber cuánto necesitas para aprobar tus cursos.</p>
        <Link to="/register"><button className="cta">Crear cuenta gratis</button></Link>
      </div>
      <div className="demo-calculator">
        ...
        <div className="input-row">
          ...
          <input placeholder="% Nota 1" value={porcentaje1} onChange={(e) => setPorcentaje1(e.target.value)} />
        </div>
        <div className="input-row">
          <input placeholder="Nota 2" value={nota2} onChange={(e) => setNota2(e.target.value)} />
          <input placeholder="% Nota 2" value={porcentaje2} onChange={(e) => setPorcentaje2(e.target.value)} />
        </div>
        {!mostrarTercera && <button onClick={() => setMostrarTercera(true)}>Agregar tercera nota (opcional)</button>}
        {mostrarTercera && (
          <div className="input-row">
            <input placeholder="Nota 3" value={nota3} onChange={(e) => setNota3(e.target.value)} />
            <input placeholder="% Nota 3" value={porcentaje3} onChange={(e) => setPorcentaje3(e.target.value)} />
          </div>
        )}
